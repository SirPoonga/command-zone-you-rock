...
4. The capture step opens a persistent Chrome profile, seeks around each timestamp, captures rendered video frames, crops the lower portion, and runs RapidOCR locally through ONNX Runtime.
...
Python 3.12 and Google Chrome are recommended. Playwright, RapidOCR, and ONNX Runtime are installed as Python dependencies. FFmpeg is needed only for the optional yt-dlp fallback backend.
...
Install FFmpeg through `winget` when needed:

```powershell
winget install --id Gyan.FFmpeg
```

Open a new PowerShell window after installation if FFmpeg is not found on `PATH`.
...
```powershell
yourock capture --candidate ftCPe3Yxztk-878-manual --all --backend browser
```
...
- Capture window around a candidate
- Chrome or Edge browser channel and persistent profile directory
- OCR crop region
- Maximum downloaded video height for the optional yt-dlp backend
