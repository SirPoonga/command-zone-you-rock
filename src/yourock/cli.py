...
def build_parser() -> argparse.ArgumentParser:
    ...
    scan = subparsers.add_parser(
        "scan",
        help="Scan playlist videos for YOU ROCK shout-outs",
    )
    ...
    scan.add_argument(
        "--backend",
        choices=("browser", "auto", "api"),
        default="browser",
        help=(
            "Discovery method. browser scans the 30 seconds before each description "
            "bookmark for the YOU ROCK graphic; auto/api use transcripts."
        ),
    )
    ...
    capture.add_argument(
        ...
        help="Read YouTube cookies from a local browser, such as chrome, edge, or firefox",
    )
    capture.add_argument(
        "--cookies-file",
        help="Path to a Netscape-format cookies.txt file (yt-dlp backend only)",
    )
    capture.add_argument(
        "--backend",
        choices=("browser", "yt-dlp"),
        help="Capture method; defaults to the backend in config.toml",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "scan":
        processed, added = scan_playlist(
            ...
            transcript_backend=args.backend,
        )
    ...
    *,
    ...
    for index, row in enumerate(selected, start=1):
        ...
        try:
            ...
            row["source"] = f"transcript+{backend or config.capture_backend}+ocr"
            row["updated_at"] = utc_now()
            print(f"  OCR text: {result.ocr_text or '(no text detected)'}")
...
def _doctor(config) -> int:
    checks = [
        ...
        ("playwright", _module_location("playwright")),
        ("rapidocr", _module_location("rapidocr")),
        ("onnxruntime", _module_location("onnxruntime")),
